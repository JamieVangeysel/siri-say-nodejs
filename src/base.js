...
class SayBase {
  ...
  speak(text, voice, speed, callback) {
    ...
    if (!text) {
      return setImmediate(() => {
        callback(new TypeError('.speak(): must provide text parameter'))
      })
    }
    ...
    this.child.addListener('exit', (code, signal) => {
      if (code === null || signal !== null) {
        return callback(new Error(`.speak(): could not talk, had an error [code: ${code}] [signal: ${signal}]`))
      }
  ...
  export(text, voice, speed, filename, callback) {
    ...
    if (!text) {
      return setImmediate(() => {
        callback(new TypeError('.export(): must provide text parameter'))
      })
    }

    if (!filename) {
      return setImmediate(() => {
        callback(new TypeError('.export(): must provide filename parameter'))
      })
    }
    ...
    this.child.addListener('exit', (code, signal) => {
      if (code === null || signal !== null) {
        return callback(new Error(`.export(): could not talk, had an error [code: ${code}] [signal: ${signal}]`))
      }
  ...
  stop(callback) {
    ...
    if (!this.child) {
      return setImmediate(() => {
        callback(new Error('.stop(): no speech to kill'))
      })
    }
